...
logical function curve_reaches_bottom_after_cylinder(x, y, n)
    use mod
    integer(4), intent(in) :: n
    real(8), intent(in) :: x(n), y(n)
    curve_reaches_bottom_after_cylinder = .false.
    if (n <= 0) return
    curve_reaches_bottom_after_cylinder = (x(n) > d0) .and. (dabs(y(n)) <= eps)
    end function curve_reaches_bottom_after_cylinder

subroutine init_top_geom_touchdown(x, y, n)
    use mod
    integer(4) :: n
    integer(4) :: index_touchdown
    real(8) :: x(n), y(n)
    
    index_touchdown = n
    do while (x(index_touchdown) > x_touchdown)
        index_touchdown = index_touchdown - 1
    end do
    x(index_touchdown) = x_touchdown
    y(index_touchdown) = d0
    call pg_allocate_boundlines(7)
    call pg_init_boundline_geompolylineds(1, ds2, x(1:index_touchdown), y(1:index_touchdown), index_touchdown - 1)
    call pg_init_boundline_geomlineds(2, ds2, x_touchdown, d0, L1*d5, d0)
    call pg_init_boundline_geomlineds(3, ds2, L1*d5, d0, L1*d5, H1)
    call pg_init_boundline_geomlineds(4, ds2, L1*d5, H1, -L1*d5, H1)
    call pg_init_boundline_geomlineds(5, ds2, -L1*d5, H1, -L1*d5, d0)
    call pg_init_boundline_geomlineds(6, ds2, -L1*d5, d0, -d1, d0)
    call pg_init_boundline_geomcircleds(7, ds, d0, d0, d1, pi, pi/2)
    end subroutine init_top_geom_touchdown

subroutine init_bottom_geom_touchdown(x, y, n)
    use mod
    integer(4) :: n
    integer(4) :: index_touchdown
    real(8) :: x(n), y(n), xt(n), yt(n)
    xt = x(size(x):1:-1)
    yt = y(size(x):1:-1)
    index_touchdown = Curves(index_extreme_particles + 1)%n
    do while (Curves(index_extreme_particles + 1)%y(index_touchdown) == d0)
        index_touchdown = index_touchdown - 1
    end do
    x_touchdown = Curves(index_extreme_particles + 1)%x(index_touchdown)
    index_touchdown = 1
    do while (xt(index_touchdown) > x_touchdown)
        index_touchdown = index_touchdown + 1
    end do
    xt(index_touchdown) = x_touchdown
    yt(index_touchdown) = d0
    call pg_allocate_boundlines(3)
    call pg_init_boundline_geomcircleds(1, ds, d0, d0, d1, pi/2, d0)
    call pg_init_boundline_geomlineds(2, ds2, d1, d0, x_touchdown, d0)
    call pg_init_boundline_geompolylineds(3, ds2, xt(index_touchdown : size(xt)), yt(index_touchdown : size(yt)), size(xt) - index_touchdown)
    end subroutine init_bottom_geom_touchdown
...
subroutine init_top_gu_touchdown()
    use mod
    real(8), allocatable :: f_tau(:)
    call pg_allocate_bound_gu
    ...
    call pg_init_boundline_gu_gen(2,4,.false.,2,1,1,1,d0,[.true.],[4],[-d1])
    call compute_f_tau(f_tau)
    call pg_init_boundline_gu_gen_var(2, 0, f_tau)
    deallocate(f_tau)
    end subroutine init_top_gu_touchdown
...
subroutine init_bottom_gu_touchdown()
    use mod
    call pg_allocate_bound_gu
    ...
    call pg_init_boundline_gu_val_const(1,1,d0)
    call pg_init_boundline_gu_val_const(2,3,d0)
    end subroutine init_bottom_gu_touchdown
...
subroutine compute_f_tau(f_tau)
    ...
    integer(4) :: n_panel, i, force_line_index
    external compute_Stocks_F 
    call pg_bind_domain(1)
    call pg_bind_bound(1)
    force_line_index = 4
    if (gsbnd%nline == 3) force_line_index = 3
    n_panel = gsbnd%line(force_line_index)%npanel
    ...
    x_c = gsbnd%xc(gsbnd%line(force_line_index)%i_begin:gsbnd%line(force_line_index)%i_end)
    y_c = gsbnd%yc(gsbnd%line(force_line_index)%i_begin:gsbnd%line(force_line_index)%i_end)
